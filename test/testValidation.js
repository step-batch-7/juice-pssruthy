...
describe('validationLib.js', () => {
  ...
  describe('isValidDateForQuery', function() {
    it('Should give true when date is not present', function() {
      assert.ok(isValidDateForQuery({ '--empId': '1234' }));
      assert.ok(isValidDateForQuery({ '--beverage': 'Orange' }));
      assert.ok(
        isValidDateForQuery({ '--beverage': 'Orange', '--empId': '1234' })
      );
    });
    it('Should give true when date is valid', () => {
      assert.ok(isValidEmpIdForQuery({ '--date': '2019-10-10' }));
      assert.ok(
        isValidEmpIdForQuery({ '--date': '2019-08-10', '--empId': '1234' })
      );
      assert.ok(
        isValidEmpIdForQuery({ '--date': '2019-10-10', '--beverage': 'Orange' })
      );
    });
    it('Should give false when date is not valid', () => {
      assert.notOk(isValidDateForQuery({ '--date': '2019-19-19' }));
      assert.notOk(isValidDateForQuery({ '--date': '2019-19-19' }));
      assert.notOk(
        isValidDateForQuery({ '--date': '2001-02-29', '--beverage': 'Orange' })
      );
    });
  });
});
